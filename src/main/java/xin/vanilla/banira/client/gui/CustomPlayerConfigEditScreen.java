...
public class CustomPlayerConfigEditScreen extends BaniraScreen {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        if (verticalAmount != 0 && contentRootPanel != null && contentRootPanel.visible() && contentRootPanel.enabled()
                && contentRootPanel.isMouseInside(mouseX, mouseY)
                && contentRootPanel.handleMouseScroll(MouseScrollEvent.of(mouseX, mouseY, verticalAmount))) {
            return true;
        }
        if (super.mouseScrolled(mouseX, mouseY, horizontalAmount, verticalAmount)) {
            return true;
        }
        if (scrollbar != null && verticalAmount != 0) {
            double newValue = Math.max(scrollbar.minValue(),
                    Math.min(scrollbar.maxValue(), scrollbar.value() - verticalAmount * 20));
