...
public final class ReadOnlyTextScreen extends BaniraScreen {
    ...
    protected void onRender(PoseStack stack, float partialTicks) {
        GuiComponent.fill(stack, 0, 0, width, height, 0x55000000);
    ...
    private void renderScrollbar(PoseStack stack, BaniraColorConfig theme) {
        if (maxScroll <= 0) {
            return;
        }
        int x = contentX + contentWidth - SCROLLBAR_WIDTH;
        GuiComponent.fill(stack, x, contentY, x + SCROLLBAR_WIDTH, contentY + contentHeight,
        ...
        GuiComponent.fill(stack, x, thumbY, x + SCROLLBAR_WIDTH, thumbY + thumbHeight,
