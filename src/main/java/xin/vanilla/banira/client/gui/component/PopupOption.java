...
public class PopupOption {
    ...
    public PopupOption addOption(@NonNull Text text) {
        ...
        List<Text> renderList = Arrays.stream(StringUtils.replaceLineBreak(text.content()).split("\n"))
    ...
    public PopupOption addOption(@NonNull String... text) {
        ...
        for (String s : text) {
            ...
            List<Text> renderList = Arrays.stream(StringUtils.replaceLineBreak(s).split("\n")).map(Text::literal).collect(Collectors.toList());
            for (int i = 0; i < renderList.size(); i++) {
                this.relationMap.put(this.renderList.size() + i, optionList.size());
            }
    ...
    public void render(MatrixStack matrixStack, KeyEventManager keyManager, MouseHelper mouseHelper) {
        ...
        if (this.adjustedY >= 0 && this.width >= 0) {
            int relativeY = (int) (mouseHelper.mouseY() - this.adjustedY - this.topPadding);
            if (this.adjustedX < mouseHelper.mouseX() && mouseHelper.mouseX() < this.adjustedX + this.width - 1) {
        ...
        AbstractGuiUtils.renderByDepth(matrixStack, EnumRenderDepth.TOOLTIP, (stack) -> {
            AbstractGuiUtils.drawRoundedRect(stack, adjustedX, adjustedY, width, height, 0x88000000, radius);
            AbstractGuiUtils.drawRoundedRectOutLineRough(stack, adjustedX, adjustedY, width, height, 1, 0xFF000000, radius);
            int lineOffset = 0;
            for (int i = 0; i < this.maxLines; i++) {
                ...
                if (index >= 0 && index < renderList.size()) {
                    ...
                    if (selectedIndex == index) {
                        AbstractGuiUtils.fill(stack, adjustedX + 1, adjustedY + topPadding + (lineOffset * (this.font.lineHeight + 1)), width - 2, this.font.lineHeight * StringUtils.getLineCount(text.content()), 0x88ACACAC);
                    }
                    FontDrawArgs args = FontDrawArgs.of(text.stack(stack).font(this.font))
                            .x(adjustedX + leftPadding).y(adjustedY + topPadding + (i * (this.font.lineHeight + 1)));
                    if (maxWidth > 0) {
                        args.maxWidth(maxWidth).position(EnumEllipsisPosition.MIDDLE);
                    }
                    AbstractGuiUtils.drawLimitedText(args);
        ...
        if (StringUtils.isNullOrEmptyEx(this.tipsKeyNames) || keyManager.isKeyPressed(this.tipsKeyNames)) {
            if (this.getSelectedIndex() >= 0 && !tipsMap.isEmpty()) {
                ...
                if (StringUtils.isNotNullOrEmpty(text.content())) {
                    FontDrawArgs args = FontDrawArgs.of(text.stack(matrixStack).font(this.font))
                            .x(mouseHelper.mouseX()).y((int) mouseHelper.mouseY());
                    AbstractGuiUtils.drawLimitedText(args);
                }
            }
        }
