...
public class ItemWidget extends BaseWidget {
    ...
    public static void renderGuiItemFlatBlit(@Nonnull PoseStack pose, @Nonnull Minecraft mc, @Nonnull ItemStack stack, int x, int y, int size) {
        ...
        BakedModel model = itemRenderer.getModel(stack, null, mc.player);
        TextureAtlasSprite sprite = model.getParticleIcon();
        int tint = ((xin.vanilla.banira.internal.mixin.accessors.MinecraftAccessor) mc)
                .banira$itemColors().getColor(stack, 0);
        if (tint != -1) {
