...
public final class QuickActionCommandsToServer implements NetworkPacket {
    ...
    public static void handle(QuickActionCommandsToServer packet, BaniraNetworkContext context) {
        context.enqueueWork(() -> {
            ...
            if (!context.isServerSide() || !(sender instanceof ServerPlayerEntity)) {
                return;
            }
            ...
            for (CustomQuickActionStep step : commandSteps(packet.steps)) {
                ...
                previousSuccess = !command.isEmpty() && BaniraCommandAccess.executePlayerCommand(
                        (ServerPlayerEntity) sender, command, 0, false);
            }
        });
        context.markHandled();
    }
