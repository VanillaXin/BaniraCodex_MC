...
public final class NBTUtils {
    ...
    public static Tag getTagByPath(Tag root, String path) {
        ...
        for (String part : parts) {
            ...
            if (indexStr != null && current instanceof CollectionTag) {
                int index = Integer.parseInt(indexStr);
                CollectionTag<?> list = (CollectionTag<?>) current;
    ...
    public static int getByte(Tag root, String path, int defaultVal) {
        Tag tag = getTagByPath(root, path);
        return (tag instanceof NumericTag) ? ((NumericTag) tag).getAsByte() : defaultVal;
    }

    public static int getShort(Tag root, String path, int defaultVal) {
        Tag tag = getTagByPath(root, path);
        return (tag instanceof NumericTag) ? ((NumericTag) tag).getAsShort() : defaultVal;
    }

    public static int getInt(Tag root, String path, int defaultVal) {
        Tag tag = getTagByPath(root, path);
        return (tag instanceof NumericTag) ? ((NumericTag) tag).getAsInt() : defaultVal;
    }

    public static float getFloat(Tag root, String path, float defaultVal) {
        Tag tag = getTagByPath(root, path);
        return (tag instanceof NumericTag) ? ((NumericTag) tag).getAsFloat() : defaultVal;
    }

    public static long getLong(Tag root, String path, long defaultVal) {
        Tag tag = getTagByPath(root, path);
        return (tag instanceof NumericTag) ? ((NumericTag) tag).getAsLong() : defaultVal;
    }

    public static double getDouble(Tag root, String path, double defaultVal) {
        Tag tag = getTagByPath(root, path);
        return (tag instanceof NumericTag) ? ((NumericTag) tag).getAsDouble() : defaultVal;
    }
