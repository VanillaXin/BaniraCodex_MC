...
public final class FabricExternalInventoryCompatibility {
    ...
    public static void init() {
        initialize("ftblibrary", "xin.vanilla.banira.internal.fabric.compat.ftblibrary.FtbLibraryCompatibility");
        ExternalInventoryButtonManager.get().refreshCurrentScreen();
    }

    private static void initialize(String modId, String className) {
        ...
        try {
            ...
            LOGGER.error("Failed to initialize external inventory button compatibility for {}", modId, exception);
        }
    }
}
