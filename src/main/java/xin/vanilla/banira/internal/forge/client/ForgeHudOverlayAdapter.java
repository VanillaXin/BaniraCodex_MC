...
/**
 * Forge 1.19.2 HUD 事件转换；不同版本只替换这一层。
 */
public final class ForgeHudOverlayAdapter {
    ...
    public static void dispatchPre(@Nonnull RenderGuiOverlayEvent.Pre event) {
        BaniraHudRenderContext context = context(event);
        HudOverlayElement element = mapElement(event.getOverlay().id().getPath());
    ...
    public static void dispatchPost(@Nonnull RenderGuiOverlayEvent.Post event) {
        BaniraHudRenderContext context = context(event);
        HudOverlayElement element = mapElement(event.getOverlay().id().getPath());
    ...
    private static BaniraHudRenderContext context(@Nonnull RenderGuiOverlayEvent event) {
        KeyValue<Integer, Integer> screen = BaniraClientRuntime.guiScaledSize();
        return new BaniraHudRenderContext(
                new xin.vanilla.banira.api.client.render.BaniraDrawContext(new ForgeBaniraDrawHandle(event.getGuiGraphics().pose()), screen.key(), screen.val(), event.getPartialTick()),
                screen.key(),
                screen.val(),
