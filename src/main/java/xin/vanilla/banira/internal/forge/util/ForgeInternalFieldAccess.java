...
/**
 * Forge 内部对象字段写入点；用于 ModInfo 等 Mixin 够不到或不适合改的加载器元数据。
 */
public final class ForgeInternalFieldAccess {
    ...
    public static boolean setObjectField(Class<?> ownerClass, Object instance, String fieldName, Object value) {
        ...
        if (field == null) {
            LOGGER.warn("Failed to locate Forge internal field {} on {}", fieldName, ownerClass.getName());
            return false;
        }
        ...
        if (Modifier.isStatic(modifiers) && Modifier.isFinal(modifiers)) {
            LOGGER.warn("Refusing to mutate static final Forge internal field {}.{}", field.getDeclaringClass().getName(), field.getName());
            return false;
        }
