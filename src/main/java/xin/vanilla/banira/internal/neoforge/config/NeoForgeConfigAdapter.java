...
public final class NeoForgeConfigAdapter {
    ...
    public static synchronized void install(IEventBus eventBus, ModContainer container) {
        ...
        if (!configListenersRegistered) {
            eventBus.addListener(NeoForgeConfigAdapter::onConfigLoading);
            eventBus.addListener(NeoForgeConfigAdapter::onConfigReloading);
            configListenersRegistered = true;
        }
    }

    /**
     * 从注解配置类构建并注册 NeoForge 配置
     *
