 ...
 * 使用示例：
 * <pre>{@code
 * ConfigHolder holder = NeoModConfigSpecBuilder.create("mymod-server", ConfigScope.SERVER)
 *   .category("base", "基础设置")
 ...
 * }</pre>
 */
public final class NeoModConfigSpecBuilder {
    ...
    private NeoModConfigSpecBuilder(String configName, ConfigScope configScope) {
     ...
     * 创建构建器
     */
    public static NeoModConfigSpecBuilder create(String configName, ConfigScope configScope) {
        return new NeoModConfigSpecBuilder(configName, configScope);
    }

    /**
     * 开始一个配置分类（可折叠块）
     */
    public NeoModConfigSpecBuilder category(String path, String... comments) {
     ...
     * 结束当前分类
     */
    public NeoModConfigSpecBuilder endCategory() {
        if (!pathStack.isEmpty()) {
            builder.pop();
            pathStack.pop();
        }
     ...
     * 定义字符串配置项
     */
    public NeoModConfigSpecBuilder define(String key, String defaultValue, String... comments) {
     ...
     * 定义布尔配置项
     */
    public NeoModConfigSpecBuilder define(String key, boolean defaultValue, String... comments) {
     ...
     * 定义整数配置项（带范围）
     */
    public NeoModConfigSpecBuilder defineInRange(String key, int defaultValue, int min, int max, String... comments) {
     ...
     * 定义长整数配置项（带范围）
     */
    public NeoModConfigSpecBuilder defineInRange(String key, long defaultValue, long min, long max, String... comments) {
     ...
     * 定义双精度浮点数配置项（带范围）
     */
    public NeoModConfigSpecBuilder defineInRange(String key, double defaultValue, double min, double max, String... comments) {
     ...
     * 定义字符串列表配置项
     */
    public NeoModConfigSpecBuilder defineList(String key, List<String> defaultValue, Predicate<Object> elementValidator, String... comments) {
    ...
    public <E extends Enum<E>> NeoModConfigSpecBuilder defineEnum(String key, E defaultValue, String... comments) {
    ...
    public ConfigHolder build(String modId) {
        ...
        return ConfigHolder.create(modId, configName, configScope, new NeoForgeConfigValueStore(spec, valueMap), new ArrayList<>(descriptors),
