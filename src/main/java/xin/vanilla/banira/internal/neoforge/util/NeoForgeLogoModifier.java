...
/**
 * NeoForge 的 modInfo logo 字段写入适配层；更高版本若字段形态变化，只替换这一层。
 */
public final class NeoForgeLogoModifier {
    private NeoForgeLogoModifier() {
    }
        ...
        try {
            ...
            for (IModInfo info : ModList.get().getMods()) {
                ...
                if (customLogo.isPresent()) {
                    NeoForgeInternalFieldAccess.setObjectField(ModInfo.class, info, field, customLogo);
                }
            }
        } catch (Exception e) {
