...
public class CreativeQuickActionInputContractTest {
    ...
    public void forgePreEventsConsumeBaniraOwnedMouseInputFirst() throws Exception {
        String bridge = source("src/main/java/xin/vanilla/banira/internal/forge/client/ForgeBaniraClientEventBridge.java");

        assertTrue(bridge.contains("@SubscribeEvent(priority = EventPriority.HIGHEST)"));
        assertTrue(bridge.contains("BaniraClientGuiService.handleMouseClicked"));
        assertTrue(bridge.contains("BaniraClientGuiService.handleMouseReleased"));
        assertTrue(bridge.contains("event.setCanceled(true)"));
    }
    ...
    public void creativeTabsDoNotHoverOrClickThroughTheBaniraOverlay() throws Exception {
        ...
        assertTrue(mixin.contains("@Mixin(CreativeScreen.class)"));
